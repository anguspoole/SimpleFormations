...
//	std::vector<sParticle> &getAliveParticles( void );
	...
	void UpdatePosition(glm::vec3 position); //used to change position of emitter
	glm::vec3 GetPosition(); //used to retrieve position of emitter

	bool active = false; //used to check if we should bother drawing at the start
