...
//			pCurMesh->velocity = pCurMesh->velocity + ( pCurMesh->accel  * deltaTime);
			...
			//glm::vec3 newVel = glm::rotate(pCurMesh->getQOrientation(), pCurMesh->velocity);
			//pCurMesh->velocity.x = newVel.x;
			//pCurMesh->velocity.y = newVel.y;
			//pCurMesh->velocity.z = newVel.z;

			//glm::vec3 newaccel = glm::rotate(pCurMesh->getQOrientation(), pCurMesh->accel);
			glm::vec3 newaccel = pCurMesh->accel;
		...
		itObjectA != vec_pObjectsToDraw.end(); itObjectA++ )
	{
				...
				itObjectB != vec_pObjectsToDraw.end(); itObjectB++ )
		{
			...
			if ( pObjectA != pObjectB )
			{
				...
				{
					//Check for collisions between player and bullets or player and enemy ships
					if((pObjectA->objectClass == cMeshObject::objClass::PLAYER) && 
						((pObjectB->objectClass == cMeshObject::objClass::ENEMYBULLET) ||
							(pObjectB->objectClass == cMeshObject::objClass::ENEMY)))
					{	
						for (int i = 0; i < pObjectA->childObjects.size(); i++)
						{
							bool colliding = TestForCollision(pObjectA, pObjectB);
							if (colliding)
							{
								pObjectB->objectClass = cMeshObject::objClass::DELETABLE;
								pObjectA->position = glm::vec3(0.0f, 0.0f, 0.0f);
								break;
							}
						}
					}
					else if ((pObjectA->objectClass == cMeshObject::objClass::PLAYERBULLET) &&
						(pObjectB->objectClass == cMeshObject::objClass::ENEMY))
					{
						bool colliding = TestForCollision(pObjectA, pObjectB);
						...
						{
							pObjectB->health -= 1;
							pObjectA->objectClass = cMeshObject::objClass::DELETABLE;

							if (pObjectB->health < 1)
							{
								pObjectB->objectClass = cMeshObject::objClass::DELETABLE;
							}
						}
...
void DeleteObjects()
{
	...
	{
		if (vec_pObjectsToDraw[i]->objectClass == cMeshObject::objClass::DELETABLE)
		{
			cMeshObject * obj = vec_pObjectsToDraw[i];
			for (int j = 0; j < aiObjects.size(); j++)
			{
				if (aiObjects[j]->objectClass == cMeshObject::objClass::DELETABLE)
				{
...
void MoveToTarget(float dt)
{
	for (int i = 0; i < vec_pObjectsToDraw.size(); i++)
	{
		if (!(vec_pObjectsToDraw[i]->atTarget))
		{
			float dist = glm::abs(glm::distance(vec_pObjectsToDraw[i]->position, vec_pObjectsToDraw[i]->targetPos));
			//std::cout << "got here, dist = " << dist << std::endl;
			if (dist > 0.1f)
			{
				glm::vec3 dir = glm::normalize(vec_pObjectsToDraw[i]->targetPos - vec_pObjectsToDraw[i]->position);
				vec_pObjectsToDraw[i]->position += (dir * 10.0f * dt);
			}
			else
			{
				vec_pObjectsToDraw[i]->atTarget = true;
			}
		}
	}
}